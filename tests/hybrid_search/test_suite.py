...
def test_search_gauges_surface_probe_availability_even_when_disabled(
    monkeypatch: "pytest.MonkeyPatch",
) -> None:
    class RecordingLogger:
        def __init__(self) -> None:
    ...
    monkeypatch.setattr(service_module, "resolve_cuvs_state", lambda *args, **kwargs: (False, False, False))
    ...
    assert dense_enabled == pytest.approx(0.0)
    assert dense_available == pytest.approx(1.0)
