...
HAS_PYALEX = "pyalex" in sys.modules
...
if HAS_PYALEX:
    ...
    class _SequentialSession:
        def __init__(self, responses: Iterable[_PdfStreamResponse]) -> None:
        ...
        def request(self, method: str, url: str, **kwargs: Any) -> _PdfStreamResponse:
    ...
    def _make_artifact(tmp_path: Path, **overrides: Any) -> WorkArtifact:
        ...
        pdf_dir = Path(params.pop("pdf_dir", tmp_path / "pdfs"))
        html_dir = Path(params.pop("html_dir", tmp_path / "html"))
        xml_dir = Path(params.pop("xml_dir", tmp_path / "xml"))
    ...
    def test_pipeline_records_resume_disabled_metadata(tmp_path: Path) -> None:
        ...
        def stub_download(
            ...
            *,
            ...
            del session, url, referer, timeout, head_precheck_passed
    ...
    def test_manifest_and_attempts_single_success(tmp_path: Path) -> None:
        ...
        def fake_download(
            ...
            *,
            ...
            del session, url, referer, timeout, context, head_precheck_passed
        ...
        records = [
            json.loads(line)
            for line in logger_path.read_text(encoding="utf-8").strip().splitlines()
        ]
        attempts = [
            entry
            for entry in records
            if entry["record_type"] == "attempt" and entry["status"] == "pdf"
        ]
        manifests = [
            entry
            for entry in records
    ...
    def test_openalex_attempts_use_session_headers(tmp_path: Path) -> None:
        ...
        def fake_download(
            ...
            *,
            ...
            del art, url, referer, timeout, context, head_precheck_passed
    ...
    def test_retry_budget_honours_max_attempts(tmp_path: Path) -> None:
        ...
        def failing_download(
            ...
            *,
            ...
            del session, artifact, referer, timeout, context, head_precheck_passed
            calls.append(url)
            return DownloadOutcome(
                classification=Classification.HTTP_ERROR,
                path=None,
