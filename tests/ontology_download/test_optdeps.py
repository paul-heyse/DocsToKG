...
def test_get_pystow_with_real_module(patch_stack, tmp_path: Path) -> None:
    ...
    patch_stack.setattr(optdeps, "_import_module", _fake_import("pystow", fake_pystow))
...
def test_get_pystow_fallback_when_missing(patch_stack, tmp_path: Path) -> None:
    """``get_pystow`` returns fallback when import fails."""

    patch_stack.setenv("PYSTOW_HOME", str(tmp_path))
    patch_stack.setattr(optdeps, "_import_module", _failing_import("pystow"))
...
def test_pystow_fallback_respects_env_var(patch_stack, tmp_path: Path) -> None:
...
def test_get_rdflib_with_real_module(patch_stack) -> None:
    """``get_rdflib`` returns imported module when available."""

    fake_graph = SimpleNamespace()
    fake_rdflib = SimpleNamespace(Graph=fake_graph)
    patch_stack.setattr(optdeps, "_import_module", _fake_import("rdflib", fake_rdflib))
...
def test_rdflib_stub_parse_and_serialize(tmp_path: Path, patch_stack) -> None:
    """Stub ``Graph`` records source and writes serialized output."""

    patch_stack.setattr(optdeps, "_import_module", _failing_import("rdflib"))
    module = optdeps.get_rdflib()
    graph = module.Graph()
...
def test_pronto_stub_behaviour(patch_stack, tmp_path: Path) -> None:
    """Pronto stub provides ``terms`` and ``dump`` methods."""

    patch_stack.setattr(optdeps, "_import_module", _failing_import("pronto"))
...
def test_owlready2_stub_load(patch_stack) -> None:
    """Owlready2 stub returns wrapper capable of loading classes."""

    patch_stack.setattr(optdeps, "_import_module", _failing_import("owlready2"))
