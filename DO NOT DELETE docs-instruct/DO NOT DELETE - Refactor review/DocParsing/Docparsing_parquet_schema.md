Awesome — here’s a **drop-in, executable module** you can paste into your repo (e.g., `src/DocsToKG/DocParsing/storage/docparse_parquet_schemas.py`). It contains:

1. **Arrow schema factories** for `Chunks` and each `Vectors` family (Dense, Sparse, Lexical).
2. A **Parquet footer contract** with builders and validators (required keys, allowed values, patterns).
3. Tiny utilities to **attach metadata**, **read** it back, and **validate** a written Parquet file conforms to the contract.

No external deps beyond `pyarrow` and `re/datetime/typing`.

---

```python
# file: src/DocsToKG/DocParsing/storage/docparse_parquet_schemas.py
...
DTYPE_ENUM = {"float32"}  # extend if you later support other storage dtypes
...
FOOTER_REQ_DENSE = (
    "docparse.dim",
    # device is nice-to-have; not strictly required for correctness:
    # "docparse.device",
)

# Sparse-only strongly recommended (vocab or hashing scheme)
FOOTER_ALT_SPARSE_ONE_OF = (
    ("docparse.sparse.vocab_id",),
    ("docparse.sparse.hash_scheme",),
)
...
FOOTER_REQ_LEXICAL = (
    ...
    "docparse.bm25.k1",
    "docparse.bm25.b",
    "docparse.stopwords_policy",  # e.g., "none" | "english" | path
    "docparse.min_df",
...
def build_footer_common(schema_version: str, cfg_hash: str, created_by: str,
...
def build_footer_dense(provider: str, model_id: str, dim: int, cfg_hash: str,
...
def build_footer_sparse(provider: str, model_id: str, cfg_hash: str,
...
def build_footer_lexical(representation: str, tokenizer_id: str,
                         k1: float, b: float,
                         stopwords_policy: str, min_df: int, max_df_ratio: float,
                         cfg_hash: str,
    ...
    meta.update({
        ...
        "docparse.model_id": model_id,
        "docparse.dtype": dtype,
        "docparse.lexical.representation": representation,  # "indices" | "terms"
...
def validate_parquet_file(path: str, family: Optional[str] = None) -> FooterValidationResult:
    ...
    if fam == "dense":
        ...
        return FooterValidationResult(ok=False, errors=(f"Unknown family '{fam}'",), warnings=tuple(warns))
...
def assert_table_matches_schema(table: pa.Table, expected: pa.Schema) -> None:
    ...
    for f in expected:
        f2 = actual.field_by_name(f.name) if f.name in actual.names else None
        if f2 is None:
...
def recommended_parquet_writer_options(dataset: str) -> Dict[str, object]:
    ...
    if dataset == "chunks":
        return {
            ...
            "data_page_size": None,  # let Arrow choose
            # "use_byte_stream_split": False  # not applicable to strings
        }
    elif dataset == "dense":
        return {
            "compression": "zstd",
            "compression_level": 5,
            "use_dictionary": {},   # no dict-encoding for float vecs
            "write_statistics": True,
            # If your pyarrow supports it, enable for float columns:
            # "use_byte_stream_split": {"vec": True}
        }
    elif dataset in ("sparse", "lexical"):
        return {
            "compression": "zstd",
            "compression_level": 5,
            "use_dictionary": {},   # lists of ints/floats don't benefit
            "write_statistics": True,
        }
    else:
...
```python
from DocsToKG.DocParsing.storage.docparse_parquet_schemas import (
    chunks_schema, dense_schema, sparse_schema_idspace,
    lexical_schema_idspace, lexical_schema_terms
)

sch_chunks = chunks_schema()
sch_dense  = dense_schema(dim=1024, fixed_size=True)
sch_sparse = sparse_schema_idspace()
sch_lex    = lexical_schema_idspace()  # or lexical_schema_terms()
```

### 2) Attach footer metadata and write

```python
import pyarrow as pa, pyarrow.parquet as pq
from DocsToKG.DocParsing.storage.docparse_parquet_schemas import (
    attach_footer_metadata, build_footer_dense,
    recommended_parquet_writer_options
)

table = pa.Table.from_pydict({...}, schema=sch_dense)
meta  = build_footer_dense(
    provider="dense.qwen_vllm",
    model_id="Qwen2-7B-Embedding@sha256:...",
    dim=1024,
    cfg_hash="ab12cd34",
    dtype="float32",
    device="cuda:0",
...
opts = recommended_parquet_writer_options("dense")
pq.write_table(table, "Vectors/family=dense/fmt=parquet/2025/10/<rel_id>.parquet", **opts)
```

*(If your `pyarrow` exposes `use_byte_stream_split`, consider enabling it for the `vec` column; the helper includes a commented hint.)*

### 3) Validate a written file’s footer

```python
from DocsToKG.DocParsing.storage.docparse_parquet_schemas import validate_parquet_file

res = validate_parquet_file(".../file.parquet")  # family inferred from footer
assert res.ok, f"Footer invalid: {res.errors}"
```

### 4) Validate a table structure before writing

```python
from DocsToKG.DocParsing.storage.docparse_parquet_schemas import assert_table_matches_schema, dense_schema

expected = dense_schema(dim=1024, fixed_size=True)
assert_table_matches_schema(table, expected)
```
...
* **Footer vs column `schema_version`:** Keep both. The column is row-visible; the footer travels with the file even if you project a subset of columns.
* **Dense fixed-size vs variable lists:** Prefer **fixed-size** for performance/compression. Fall back to variable lists only if dim changes within the same file (uncommon).
* **Sparse & Lexical invariants:** Enforce `nnz == len(indices|terms) == len(weights)` in your writer layer; the schema alone can’t guarantee list lengths match.
* **Bloom filters / row-group size:** Keep those choices in your writer; the `recommended_parquet_writer_options` helper is only a hint, because exact kwargs vary by `pyarrow` version.
* **Metadata types:** Parquet key/value metadata values are **bytes** internally; the helper handles UTF-8 encoding/decoding.

---

If you’d like, I can also provide a **tiny “schema check” CLI** (Typer command) that:

* opens a Parquet file (or glob),
* prints schema and footer,
* validates against the contract above,
* and returns non-zero on errors (good for CI).
