Here’s a drop-in loader you can add as:

`src/DocsToKG/ContentDownload/breakers_loader.py`

It reads your YAML (optional), applies **env & CLI overlays**, and returns a fully-resolved `BreakerConfig` with **host keys normalized to lowercase punycode** so the `BreakerRegistry` sees stable keys.

> It uses the dataclasses you already have in `breakers.py` (from the skeleton we built earlier). No extra dependencies beyond PyYAML.

```python
# File: src/DocsToKG/ContentDownload/breakers_loader.py
"""
Breaker config loader: YAML -> BreakerConfig with env/CLI overlays,
host key normalization (lowercase + punycode), and basic validation.
...
cfg = load_breaker_config(
    ...
    cli_defaults_override=None,  # e.g. "fail_max:5,reset:60"
    cli_classify_override=None,  # e.g. "failure=429,500,502,503,504,408 neutral=401,403,404,410,451"
    cli_rolling_override=None,   # e.g. "enabled:true,window:30,thresh:6,cooldown:60"
)
"""
...
### What this loader covers

* **YAML → BreakerConfig**: parses `defaults`, `classify`, `half_open`, `advanced.rolling`, `hosts`, and `resolvers` sections (matching the schema we drafted).
* **Env overlays**:

  * `DOCSTOKG_BREAKER__HOST=fail_max:...,reset:...,retry_after_cap:...`
  * `DOCSTOKG_BREAKER_ROLE__HOST__ROLE=...`  (ROLE ∈ `metadata|landing|artifact`)
  * `DOCSTOKG_BREAKER_RESOLVER__NAME=...`
  * `DOCSTOKG_BREAKER_DEFAULTS=...`, `DOCSTOKG_BREAKER_CLASSIFY=...`, `DOCSTOKG_BREAKER_ROLLING=...`
* **CLI overlays**:

  * `--breaker`, `--breaker-role`, `--breaker-resolver`, `--breaker-defaults`, `--breaker-classify`, `--breaker-rolling`
* **Host key normalization**: lowercase + **punycode** with `idna` codec.
* **Validation**: enforces `fail_max>=1`, `reset>0`, and `retry_after_cap>0`; validates role overrides if present.

> If you’d like, I can also add tiny `argparse` helpers to parse the CLI flags into the sequences this loader expects, plus a 15-line “print effective policy table” utility for your startup logs.
