...
def migrate_resolver(resolver_name: str) -> bool:
    ...
    content = re.sub(r"class (\w+)\((RegisteredResolver|ApiResolverBase)\):", r"class \1:", content)
...
'''
            content = (
                content[:import_section_end] + resolver_result_def + content[import_section_end:]
            )
    ...
    content = re.sub(r'config: "ResolverConfig"', "config: Any", content)
    ...
    if "config: Any" in content and "from typing import" in content:
        content = re.sub(
            ...
            lambda m: (
                f"from typing import {m.group(1)}, Any" if "Any" not in m.group(1) else m.group(0)
            ),
            content,
            count=1,
        )
