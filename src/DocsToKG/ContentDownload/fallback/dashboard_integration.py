...
class DashboardExporter:
    ...
    def export_for_prometheus(self, period: str = "24h") -> str:
        ...
        for source, stats in source_stats.items():
            lines.append(f'fallback_source_success_rate{{source="{source}"}} {stats["success_rate"]}')
            lines.append(f'fallback_source_error_rate{{source="{source}"}} {stats["error_rate"]}')
            lines.append(f'fallback_source_timeout_rate{{source="{source}"}} {stats["timeout_rate"]}')
...
class RealTimeMonitor:
    ...
    def get_trend(self, period: str = "24h") -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate_change": second_stats.get("success_rate", 0) - first_stats.get("success_rate", 0),
            "latency_change_percent": (
                (second_stats.get("avg_elapsed_ms", 0) - first_stats.get("avg_elapsed_ms", 0)) /
                max(first_stats.get("avg_elapsed_ms", 1), 1) * 100
            ),
        }
