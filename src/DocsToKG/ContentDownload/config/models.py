...
class HttpClientConfig(BaseModel):
    ...
    # Timeout settings
    timeout_connect_s: float = Field(default=5.0, description="Connect timeout (seconds)")
    timeout_read_s: float = Field(default=30.0, description="Read timeout (seconds)")
    timeout_write_s: float = Field(default=30.0, description="Write timeout (seconds)")
    timeout_pool_s: float = Field(default=5.0, description="Pool acquire timeout (seconds)")

    # Pool settings
    max_connections: int = Field(default=64, description="Max connections")
    max_keepalive_connections: int = Field(default=20, description="Max keepalive connections")
    keepalive_expiry_s: float = Field(default=30.0, description="Keepalive expiry (seconds)")
    ...
    connect_retries: int = Field(default=2, description="Transport-level connect retries")
    retry_backoff_base: float = Field(default=0.1, description="Backoff base (seconds)")
    retry_backoff_max: float = Field(default=2.0, description="Max backoff (seconds)")
