...
|----------|---------|---------|
| `ONTOLOGY_FETCHER_CONFIG` | Path to override the config file loaded by the CLI. | Auto-discovered from `~/.data/ontology-fetcher/configs/sources.yaml` when present. |
| `ONTOFETCH_LOG_DIR` | Redirect JSONL logs generated by `logging_utils`. | `~/.data/ontology-fetcher/logs`. |
| `LOCAL_ONTOLOGY_DIR` | Override ontology artifact destination. | `~/.data/ontology-fetcher/ontologies`. |
...
These directories inherit from `pystow`'s application root, so setting `PYSTOW_HOME` adjusts the parent of `configs`, `cache`, `logs`, and `ontologies` in tandem with the defaults above.
