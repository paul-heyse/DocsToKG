...
- **Download runtime** – `io.network.StreamingDownloader` provides shared `SESSION_POOL` instances, DNS caching, redirect auditing, resume support, Retry-After aware rate limiting via `io.rate_limit`, and HTTP allowlist enforcement (`validate_url_security`) that honours documentation networks while blocking private ranges by default.
