    ...
    if args.json:
        ...
        if cmd == "latest":
            ...
            if result.get("output"):
                ...
                by_validator = result.get("by_validator", {})
                if by_validator:
                    print("  By validator:")
                    for validator, stats in by_validator.items():
                        print(f"    {validator:15} {stats['passed']:3}/{stats['total']:3}")

        elif cmd == "export-inventory":
            print(f"Artifact Inventory: {result.get('version_id')}")
            if result.get("output"):
                print(f"  Exported to: {result.get('output')}")
            else:
                summary = result.get("summary", {})
                print(f"  Total artifacts: {summary.get('total_artifacts', 0)}")
                print(f"  Total bytes: {summary.get('total_bytes', 0) / (1024**3):.2f} GB")
                print(f"  Files extracted: {summary.get('total_files_extracted', 0)}")
                print(
                    f"  Extracted bytes: {summary.get('total_extracted_bytes', 0) / (1024**2):.2f} MB"
                )
