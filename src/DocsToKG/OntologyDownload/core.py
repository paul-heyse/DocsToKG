...
def fetch_one(
    ...
    *,
    ...
    plan, resolver_name = _resolve_plan_with_fallback(spec, active_config, adapter)
    ...
    _ensure_license_allowed(plan, active_config, effective_spec)
    ...
    destination, version, base_dir = _build_destination(effective_spec, plan, active_config)
    manifest_path = base_dir / "manifest.json"

    previous_manifest = None
    if not force:
        STORAGE.ensure_local_version(effective_spec.id, version)
    ...
    try:
        ...
        raise OntologyDownloadError(f"Download failed for '{effective_spec.id}': {exc}") from exc
...
def plan_one(
    ...
    *,
    ...
    _ensure_license_allowed(plan, active_config, effective_spec)
    return PlannedFetch(spec=effective_spec, resolver=resolver_name, plan=plan)
