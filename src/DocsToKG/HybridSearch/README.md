...
4. **Stream ingestion** – Construct `ChunkIngestionPipeline(faiss_index=..., opensearch=..., registry=..., observability=...)` and call `upsert_documents(...)` with DocParsing manifests. The pipeline normalises BM25/SPLADE/dense payloads, lazily streams vector artefacts (raising `IngestError` when UUID order drifts beyond the configured `vector_cache_limit`), keeps sparse+dense stores in sync, and records metrics. Setting `vector_cache_limit` to `0` or a negative value no longer disables the guard; the value is coerced to fail immediately on drift, so ingestion aborts as soon as misordered vectors are observed.
